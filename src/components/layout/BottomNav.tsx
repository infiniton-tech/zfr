...
const CONTACT_DEFAULTS = {
  ...
  contact_email: "support@arjo.com",
  contact_hours: "10:00 AM - 8:00 PM (Daily)",
};
...
export function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-neutral-200 h-[64px] flex items-center justify-around px-2 md:hidden shadow-[0_-4px_12px_rgba(0,0,0,0.06)] pb-safe">
      
      {/* Category (Trigger Sidebar Sheet) */}
      <Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}>
        <SheetTrigger className="flex flex-col items-center justify-center flex-1 h-full text-[10px] font-medium text-neutral-500 hover:text-black transition-colors focus:outline-none">
          <Menu className="w-5 h-5 mb-0.5 text-neutral-700" />
          <span>Category</span>
        </SheetTrigger>
      ...
      >
        <Phone className="w-5 h-5 mb-0.5 text-neutral-700" />
        <span>Contact</span>
      </button>

      {/* Home (Central Terracotta overlapping Circle) */}
      <Link href="/" className="relative -top-3 flex flex-col items-center justify-center z-50">
        <div className="w-[56px] h-[56px] rounded-full bg-[#C49A85] flex flex-col items-center justify-center shadow-lg border-4 border-white hover:bg-[#b38a75] transition-colors">
          <Home className="w-5 h-5 text-white" />
          <span className="text-[9px] font-semibold text-white mt-0.5 leading-none">Home</span>
        </div>
      </Link>

      {/* Cart (Trigger Cart Sheet) */}
      <Sheet open={cartOpen} onOpenChange={setCartOpen}>
        <SheetTrigger className="flex flex-col items-center justify-center flex-1 h-full text-[10px] font-medium text-neutral-500 hover:text-black transition-colors focus:outline-none relative">
          <div className="relative">
            <ShoppingBag className="w-5 h-5 mb-0.5 text-neutral-700" />
            {count > 0 && (
              <span className="absolute -top-1 -right-1.5 w-4 h-4 bg-black text-white text-[8px] font-bold flex items-center justify-center rounded-full">
                {count}
              </span>
            )}
          </div>
      ...
      {contactOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full border border-neutral-100 shadow-2xl relative">
            <h3 className="text-lg font-bold text-neutral-900 mb-4 uppercase tracking-wider">Contact Us</h3>
            <div className="space-y-3 text-sm text-neutral-600">
              <p className="flex items-center gap-2">
                <strong className="text-neutral-900">Phone:</strong> {contact.contact_phone}
              </p>
              <p className="flex items-center gap-2">
                <strong className="text-neutral-900">Email:</strong> {contact.contact_email}
              </p>
              <p className="flex items-center gap-2">
                <strong className="text-neutral-900">Hours:</strong> {contact.contact_hours}
              </p>
            </div>
            <button
              onClick={() => setContactOpen(false)}
              className="mt-6 w-full bg-black text-white py-2 text-xs font-semibold tracking-widest hover:bg-neutral-800 transition-colors uppercase"
            >
              Close
            </button>
          </div>
